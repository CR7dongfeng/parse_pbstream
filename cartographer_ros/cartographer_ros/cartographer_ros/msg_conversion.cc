...
#define init_cloud_by_ros_msg(msg, pcl_cloud, res_cloud) \
  {                                                      \
    pcl::fromROSMsg(msg, pcl_cloud);                     \
    res_cloud.points.reserve(pcl_cloud.size());          \
    res_cloud.intensities.reserve(pcl_cloud.size());     \
  }

#define add_point_to_cloud(point, intensity, time, res_cloud) \
  {                                                           \
    Eigen::Vector3f p(point.x, point.y, point.z);             \
    if (p.norm() < 2) continue;                               \
    res_cloud.points.push_back({p, time});                    \
    res_cloud.intensities.push_back(intensity);               \
  }
...
ToPointCloudWithIntensities(const sensor_msgs::PointCloud2& msg) {
  ...
  // weiji: time field records the delta time from the beginning of the packet
  // weiji: filter too close point
  if (PointCloud2HasField(msg, "intensity")) {
    if (PointCloud2HasField(msg, "time")) {
      pcl::PointCloud<PointXYZIT> pcl_point_cloud;
      init_cloud_by_ros_msg(msg, pcl_point_cloud, point_cloud);
      for (const auto& point : pcl_point_cloud) {
        add_point_to_cloud(point, point.intensity, point.time, point_cloud)
      }
      output_log = "Received point cloud msg with intensity & time";
    }
    else {
      pcl::PointCloud<pcl::PointXYZI> pcl_point_cloud;
      init_cloud_by_ros_msg(msg, pcl_point_cloud, point_cloud);
      for (const auto& point : pcl_point_cloud) {
        add_point_to_cloud(point, point.intensity, 0.f, point_cloud)
      }
      output_log = "Received point cloud msg with intensity, but no time";
    }
  } else {
    ...
    if (PointCloud2HasField(msg, "time")) {
      ...
      init_cloud_by_ros_msg(msg, pcl_point_cloud, point_cloud);
      for (const auto& point : pcl_point_cloud) {
        add_point_to_cloud(point, 1.f, point.time, point_cloud)
      }
      output_log = "Received point cloud msg with time, but no intensity";
    } else {
      pcl::PointCloud<pcl::PointXYZ> pcl_point_cloud;
      init_cloud_by_ros_msg(msg, pcl_point_cloud, point_cloud);
      for (const auto& point : pcl_point_cloud) {
        add_point_to_cloud(point, 1.f, 0.f, point_cloud)
      }
      output_log = "Received point cloud msg without both intensity & time";
    }
  }
  if (first_msg) {
    first_msg = false;
