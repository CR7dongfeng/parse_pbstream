...
void IntensityToColorPointsProcessor::Process(
  ...
  if (!batch->intensities.empty() &&
    ...
    batch->colors.clear();
    // weiji: change intensity rather than give color
    for (float& intensity : batch->intensities) {
      float tmp = (intensity - min_intensity_) / (max_intensity_ - min_intensity_) * 255;
      intensity = tmp < 0? 0: (tmp > 255? 255: tmp);
