...
void ConstraintBuilder3D::MaybeAddGlobalConstraint(
  ...
  // weiji: push_back() a new default constraint
  constraints_.emplace_back();
  kQueueLengthMetric->Set(constraints_.size());
  auto* const constraint = &constraints_.back();
  const auto* scan_matcher = DispatchScanMatcherConstruction(submap_id, submap);
...
void ConstraintBuilder3D::ComputeConstraint(
  ...
  // weiji: if constraint between node and submap belongs to "long distance"
  bool long_type = false;
  int submap_of_node = node_id.node_index / options_.submap_node_size();
  int seq = rand();
  float min_score = options_.min_score();
  if (match_full_submap) {
    kGlobalConstraintsSearchedMetric->Increment();
    match_result =
        submap_scan_matcher.fast_correlative_scan_matcher->MatchFullSubmap(
            seq, global_node_pose.rotation(), global_submap_pose.rotation(),
    ...
    if (match_result != nullptr) {
      ...
      LOG(INFO) << "can not find GLOBAL constraint_match " << seq
                << " between node " << node_id << " at "
                << transform::DebugRigid3(global_node_pose) 
                << " and submap " << submap_id << " at "
                << transform::DebugRigid3(global_submap_pose);
      return;
    }
  } else {
    kConstraintsSearchedMetric->Increment();
    transform::Rigid3d opt_node_pose = global_node_pose;
    // add by weiji: only allow new-node match previous-submap
    //if (abs(submap_of_node - submap_id.submap_index) <= 1)
    if (submap_of_node - submap_id.submap_index <= 1)
      return;
    if (options_.long_distance_submap_count() > 0) {
      long_type = abs(submap_of_node - submap_id.submap_index) >
                  options_.long_distance_submap_count();
      if (long_type) {
        min_score = options_.long_distance_min_score();
        int gap = submap_of_node - last_find_long_submap_index;
        if (gap > 0)
          min_score -= (gap > 6? 5: gap - 1) * 0.02f;
/*      Eigen::Vector3d rot_node = transform::RotationQuaternionToEulerVector(
            global_node_pose.rotation());
        Eigen::Vector3d rot_submap = transform::RotationQuaternionToEulerVector(
            global_submap_pose.rotation());
        Eigen::Quaterniond q = transform::RollPitchYaw(
            rot_submap.x(), rot_submap.y(), rot_node.z());
        opt_node_pose = transform::Rigid3d(global_node_pose.translation(), q);*/
      }
    }
    match_result = submap_scan_matcher.fast_correlative_scan_matcher->Match(
        seq, opt_node_pose, global_submap_pose, *constant_data, min_score);
    if (match_result != nullptr) {
      if (long_type && submap_of_node > last_find_long_submap_index) {
        last_find_long_submap_index = submap_of_node;
        LOG(INFO) << "last find long submap index: "
                  << last_find_long_submap_index;
      }
      // We've reported a successful local match.
      CHECK_GT(match_result->score, min_score);
      ...
      if (long_type) {
        LOG(INFO) << "can not find LONG constraint_match "
                  << seq << " between node " << node_id
                  //<< " at " << transform::DebugRigid3(opt_node_pose) 
                  << " and submap " << submap_id 
                  //<< " at " << transform::DebugRigid3(global_submap_pose)
  ...
  Constraint::Tag detail = long_type? Constraint::INTER_SUBMAP_LONG: Constraint::INTER_SUBMAP;
  constraint->reset(new Constraint{
      submap_id,
      node_id,
      {constraint_transform, options_.loop_closure_translation_weight(),
       options_.loop_closure_rotation_weight()},
      Constraint::INTER_SUBMAP, detail,
      match_result->score});

  if (options_.log_matches()) {
    std::ostringstream info;
    if (long_type) info << "LONG ";
    info << "constraint_match " << seq << ", Node " << node_id << " with "
         << constant_data->high_resolution_point_cloud.size()
         << " points on submap " << submap_id << std::fixed;
    if (long_type) info << " with min_score " << min_score;
    if (match_full_submap) {
      info << " matches";
    } else {
...
void ConstraintBuilder3D::RunWhenDoneCallback() {
  ...
  std::unique_ptr<std::function<void(const Result&)>> callback;
  {
    ...
    for (const std::unique_ptr<Constraint>& constraint : constraints_) {
      ...
      tmp_result.push_back(*constraint);
    }
    // add by weiji: keep only the best match belonging to the same submap
    for (const Constraint& cand : tmp_result) {
      bool valid = true;
      for (const Constraint& other : tmp_result) {
        if (other.node_id.node_index != cand.node_id.node_index &&
            other.submap_id.submap_index == cand.submap_id.submap_index &&
            other.score > cand.score) {
          valid = false;
          LOG(INFO) << "drop dumplicate constraint between node "
                    << cand.node_id << " and submap " << cand.submap_id;
          break;
        }
        if (abs(other.submap_id.submap_index - cand.submap_id.submap_index) <= 1 &&
            other.score > cand.score * 1.2f) {
          valid = false;
          LOG(INFO) << "drop low-score constraint between node "
                    << cand.node_id << " and submap " << cand.submap_id;
          break;
        }
      }
    ...
    LOG(INFO) << count_times << " times: " << constraints_.size() << " computations resulted in "
