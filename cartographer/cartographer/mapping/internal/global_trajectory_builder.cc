...
class GlobalTrajectoryBuilder : public mapping::TrajectoryBuilderInterface {
 public:
  ...
  void AddSensorData(
    ...
    // weiji: 0 means empty msg just for holding frequency
    if (fixed_frame_pose.status != 0) {
      //std::cout << "add fixed pose, status " << fixed_frame_pose.status
      //          << ", pos " << (fixed_frame_pose.pose.has_value()? 
      //             transform::DebugRigid3(fixed_frame_pose.pose.value()): "null")
      //          << std::endl;
