...
LocalTrajectoryBuilder3D::LocalTrajectoryBuilder3D(
    ...
    : options_(options),
      ...
      range_data_collator_(expected_range_sensor_ids) {
        // weiji: export poses
        fout_ = new std::ofstream("poses.out");
      }
...
std::unique_ptr<transform::Rigid3d> LocalTrajectoryBuilder3D::ScanMatch(
    ...
    const sensor::OdometryData* ptr_odom_observation,
  ...
  // weiji: compute the predict pose inside the matching submap
  ...
  // weiji: compute delta by local pose
  sensor::OdometryData odom_local;
  const sensor::OdometryData* ptr_odom_local = NULL;
  if (ptr_odom_observation != NULL)
  {
    odom_local.pose =
        matching_submap->local_pose().inverse() * ptr_odom_observation->pose;
    odom_local.variance = ptr_odom_observation->variance;
    odom_local.onlyGravity = ptr_odom_observation->onlyGravity;
    ptr_odom_local = &odom_local;
  }

  transform::Rigid3d pose_observation_in_submap;
  ceres::Solver::Summary summary;
  ceres_scan_matcher_->Match(
      // weiji: initial_ceres_pose may be modified in 'realtime match' above 
...
void LocalTrajectoryBuilder3D::AddImuData(const sensor::ImuData& imu_data) {
  ...
  // weiji: add this imu_start_time for logging
  imu_start_time_ = imu_data.time;
  ...
  extrapolator_ = mapping::PoseExtrapolator::InitializeWithImu(
      ...
      options_.use_wheel_odometry());
}

// Returns 'MatchingResult' when range data accumulation completed,
// otherwise 'nullptr'.
// `TimedPointCloudData::time` is when the last point in `range_data` was
// acquired, `TimedPointCloudData::ranges` contains the relative time of point
// with respect to `TimedPointCloudData::time`.
std::unique_ptr<LocalTrajectoryBuilder3D::MatchingResult>
LocalTrajectoryBuilder3D::AddRangeData(
    const std::string& sensor_id,
    const sensor::TimedPointCloudData& unsynchronized_data) {
  // weiji: synchronize until we get one packet from each lidar 
  //        return TimedPointCloudOriginData
  ...
  // wuweiji: half size voxel_filter
  ...
  for (const auto& hit : hits) {
    ...
    // add by weiji: add odom support
    transform::Rigid3d pose;
    if (extrapolator_->EnableOdom() && !options_.use_wheel_odometry()) {
      sensor::OdometryData odom = extrapolator_->ExtrapolateOdometry(time_point);
      pose = odom.pose;
    } else {
      pose = extrapolator_->ExtrapolatePose(time_point);
    }
    hits_poses.push_back(pose.cast<float>());
  }
  ...
  // weiji: add point in packets into accumulated_range_data_
  for (size_t i = 0; i < hits.size(); ++i) {
    sensor::RangefinderPoint hit_in_local =
        hits_poses[i] * sensor::ToRangefinderPoint(hits[i].point_time);
    // weiji: to compute the actural range
    //        use origin_index to separate diffrent lidars
...
LocalTrajectoryBuilder3D::AddAccumulatedRangeData(
  ...
  // weiji: dump first cloud
  /*static bool first_cld = true;
  if (first_cld) {
    first_cld = false;
    *fout_ << "first cloud dump: " << std::endl; 
    for (auto& p : filtered_range_data_in_tracking.returns) {
      *fout_ << p.position.x() << "," << p.position.y() << "," << p.position.z()
             << std::endl;
    }
  }*/

  // weiji: get imu orientation
  const Eigen::Quaterniond gravity_alignment =
      extrapolator_->EstimateGravityOrientation(time);
  *fout_ << common::ToSeconds(time - imu_start_time_) << ", imu orientation: "
         << transform::DebugQuaternion(gravity_alignment) << std::endl;

  // weiji: get pose prediction
  const transform::Rigid3d pose_prediction =
      extrapolator_->ExtrapolatePose(time);
  double time_sec = common::ToSeconds(time - imu_start_time_);
  //extrapolator_->PrintState(time_sec, fout_);
  *fout_ << time_sec << ", predict: "
         << transform::DebugRigid3(pose_prediction) << std::endl;

  // weiji: get odom / gravity observation
  sensor::OdometryData odom_observation;
  const sensor::OdometryData* ptr_odom_observation = &odom_observation;
  if (extrapolator_->EnableOdom() && !options_.use_wheel_odometry()) {
    odom_observation = extrapolator_->ExtrapolateOdometry(time);
    *fout_ << time_sec << ", odom: " 
           << transform::DebugRigid3(odom_observation.pose) << std::endl;
  } else {
    odom_observation.time = time;
    odom_observation.pose = transform::Rigid3d::Rotation(gravity_alignment);
    odom_observation.variance = 1;
    odom_observation.onlyGravity = true;
  }

  const auto scan_matcher_start = std::chrono::steady_clock::now();

  // weiji: start with max_length, then try half of max_length repeatly
  //        until the size of point_cloud is bigger than min_num
  ...
  std::unique_ptr<transform::Rigid3d> pose_estimate =
      ScanMatch(pose_prediction, ptr_odom_observation,
                low_resolution_point_cloud_in_tracking,
                high_resolution_point_cloud_in_tracking);
  ...
  // weiji: add pose estimate
  *fout_ << common::ToSeconds(time - imu_start_time_) << ", estimate: "
         << transform::DebugRigid3(*pose_estimate) << std::endl;
  ...
  // weiji: transfer points from base into local
  sensor::RangeData filtered_range_data_in_local = sensor::TransformRangeData(
      filtered_range_data_in_tracking, pose_estimate->cast<float>());

  // weiji: add into submap, using pose & imu orientation
