...
class RotationDeltaCostFunctor3D {
 public:
  static ceres::CostFunction* CreateAutoDiffCostFunction(
      const double scaling_factor, const double scaling_factor_yaw,
    ...
    return new ceres::AutoDiffCostFunction<RotationDeltaCostFunctor3D,
        ...
        new RotationDeltaCostFunctor3D(scaling_factor, scaling_factor_yaw,
                                       target_rotation));
  }
  ...
  bool operator()(const T* const rotation_quaternion, T* residual) const {
    ...
    // weiji: change residual from quaternion to axis_angel
    ...
    std::array<T, 3> axis_angel;
    ceres::QuaternionToAngleAxis(delta.data(), axis_angel.data());
    residual[0] = scaling_factor_ * axis_angel[0];
    residual[1] = scaling_factor_ * axis_angel[1];
    residual[2] = scaling_factor_yaw_ * axis_angel[2];
    return true;
  }
 ...
 private:
  ...
  explicit RotationDeltaCostFunctor3D(const double scaling_factor,
                                      const double scaling_factor_yaw,
                                      const Eigen::Quaterniond& target_rotation)
      : scaling_factor_(scaling_factor),
        scaling_factor_yaw_(scaling_factor_yaw) {
  ...
  const double scaling_factor_yaw_;
