...
proto::CeresScanMatcherOptions3D CreateCeresScanMatcherOptions3D(
  ...
  options.set_rotation_weight_yaw(
      parameter_dictionary->GetDouble("rotation_weight_yaw"));
  options.set_odom_translation_weight(
      parameter_dictionary->GetDouble("odom_translation_weight"));
  options.set_odom_translation_weight_z(
      parameter_dictionary->GetDouble("odom_translation_weight_z"));
  options.set_odom_rotation_weight(
      parameter_dictionary->GetDouble("odom_rotation_weight"));
  options.set_odom_rotation_weight_yaw(
      parameter_dictionary->GetDouble("odom_rotation_weight_yaw"));
...
void CeresScanMatcher3D::Match(
    ...
    const sensor::OdometryData* ptr_odom_local,
  ...
  for (size_t i = 0; i != point_clouds_and_hybrid_grids.size(); ++i) {
    ...
    // weiji: transfer points in point_cloud using translation and rotation
    //        then compute their probability in hybrid_grid 
  ...
  CHECK_GT(options_.translation_weight_z(), 0.);
  problem.AddResidualBlock(
  ...
  CHECK_GT(options_.rotation_weight_yaw(), 0.);
  problem.AddResidualBlock(
      RotationDeltaCostFunctor3D::CreateAutoDiffCostFunction(
          options_.rotation_weight(), options_.rotation_weight_yaw(),
          initial_pose_estimate.rotation()),
      nullptr /* loss function */, ceres_pose.rotation());

  // weiji: add odom residual block
  if (ptr_odom_local != NULL) {
    double scale = 1 / ptr_odom_local->variance;
    CHECK_GT(scale, 0.);
    if (!ptr_odom_local->onlyGravity) {
        CHECK_GT(options_.odom_translation_weight(), 0.);
        CHECK_GT(options_.odom_translation_weight_z(), 0.);
        problem.AddResidualBlock(
            TranslationDeltaCostFunctor3D::CreateAutoDiffCostFunction(
                options_.odom_translation_weight() * scale,
                options_.odom_translation_weight_z() * scale,
                ptr_odom_local->pose.translation()),
            nullptr /* loss function */, ceres_pose.translation());
    }
    CHECK_GT(options_.odom_rotation_weight(), 0.);
    CHECK_GE(options_.odom_rotation_weight_yaw(), 0.);
    problem.AddResidualBlock(
        RotationDeltaCostFunctor3D::CreateAutoDiffCostFunction(
            options_.odom_rotation_weight() * scale,
            options_.odom_rotation_weight_yaw() * scale,
            ptr_odom_local->pose.rotation()),
        nullptr /* loss function */, ceres_pose.rotation());
  }

  ceres::Solve(ceres_solver_options_, &problem, summary);

  // weiji: pose_estimate means pose_observation_in_submap,
  //        final pose is "submap->local_pose() * pose_estimate"
