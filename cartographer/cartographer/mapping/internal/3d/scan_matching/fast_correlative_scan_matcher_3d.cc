...
FastCorrelativeScanMatcher3D::FastCorrelativeScanMatcher3D(
    const HybridGrid& hybrid_grid,  // weiji: high_resolution_hybrid_grid
...
FastCorrelativeScanMatcher3D::Match(
    int seq, const transform::Rigid3d& global_node_pose,
  ...
  return MatchWithSearchParameters(
      seq, search_parameters, global_node_pose.cast<float>(),
...
FastCorrelativeScanMatcher3D::MatchFullSubmap(
    int seq, const Eigen::Quaterniond& global_node_rotation,
...
FastCorrelativeScanMatcher3D::MatchWithSearchParameters(
  ...
  // add by weiji, long distance match details
  if (min_score <= 0.3) {
    int scan_size = discrete_scans.size();
    int cand_size = lowest_resolution_candidates.size();
    float max_rotational_score = 0;
    float max_score = 0;
    if (lowest_resolution_candidates.size() > 0) {
      int scan_index = lowest_resolution_candidates[0].scan_index;
      max_score = lowest_resolution_candidates[0].score;
      max_rotational_score = discrete_scans[scan_index].rotational_score;
    }
    //LOG(INFO) << "count LONG constraint_match " << seq
    //          << ", scan size " << scan_size << " with best score " << max_rotational_score
    //          << ", low resulotion cand size " << cand_size << " with best score " << max_score;
  }
...
std::vector<DiscreteScan3D> FastCorrelativeScanMatcher3D::GenerateDiscreteScans(
    const FastCorrelativeScanMatcher3D::SearchParameters& search_parameters,
    const sensor::PointCloud& point_cloud,  // weiji: origin point cloud
    const Eigen::VectorXf& rotational_scan_matcher_histogram,
    const Eigen::Quaterniond& gravity_alignment,
    const transform::Rigid3f& global_node_pose,
    const transform::Rigid3f& global_submap_pose) const {
  std::vector<DiscreteScan3D> result;
  // weiji: set a minimum value = 3 * resolution_ of high_resolution_hybrid_grid
  // to make sure that the std::acos() below is defined
  ...
  // weiji: count the similarity of two histogram when changing yaw
