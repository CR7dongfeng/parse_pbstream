...
// weiji: view function comments in https://zhuanlan.zhihu.com/p/50290659 
...
// weiji: insertion_submaps contains both active (front/back) submaps
NodeId PoseGraph3D::AddNode(
    std::shared_ptr<const TrajectoryNode::Data> constant_data,
    const int trajectory_id,
    const std::vector<std::shared_ptr<const Submap3D>>& insertion_submaps) {
  // weiji: count the global pose
  const transform::Rigid3d optimized_pose(
      GetLocalToGlobalTransform(trajectory_id) * constant_data->local_pose);
  // weiji: trajectory_nodes_.Append()
...
void PoseGraph3D::AddWorkItem(
  ...
  absl::MutexLock locker(&work_queue_mutex_);
  // weiji: if work_queue_ is empty, create a WorkQueue
  ...
  // weiji: add work_item to the end of the work_queue_, will be called
...
WorkItem::Result PoseGraph3D::ComputeConstraintsForNode(
  ...
  std::set<NodeId> newly_finished_submap_node_ids;
  {
    ...
    // weiji: look up submap_id in optimization_problem_->submap_data()
    // view function comments in https://zhuanlan.zhihu.com/p/50290659
    ...
    // weiji: add local pose & global pose into TrajectoryNode
    optimization_problem_->AddTrajectoryNode(
        matching_id.trajectory_id,
        optimization::NodeSpec3D{constant_data->time, local_pose, global_pose});
    // weiji: add constraint for both active submaps
    for (size_t i = 0; i < insertion_submaps.size(); ++i) {
      ...
      // weiji: KEY CONSTRAINT -> node's rigid3d in a submap
      ...
      data_.constraints.push_back(Constraint{
          ...
          // INTRA_SUBMAP means 'in submap'; INTER_SUBMAP means 'between submap' 
    }
    ...
    // weiji: set newly_finished_submap's state to kFinished
  ...
  // weiji: try to add CONSTRAINT between this node and all submaps 
