...
class PoseExtrapolator {
 public:
  ...
  static std::unique_ptr<PoseExtrapolator> InitializeWithImu(
      ...
      const sensor::ImuData& imu_data, bool use_wheel_odometry = true);
  ...
  sensor::OdometryData ExtrapolateOdometry(common::Time time);

  // Returns the current gravity alignment estimate as a rotation from
  // the tracking frame into a gravity aligned frame.
  Eigen::Quaterniond EstimateGravityOrientation(common::Time time);

  void PrintState(double time_sec, std::ofstream* fout) {
    //const Eigen::Quaterniond tmp = extrapolation_imu_tracker_->orientation();
    //*fout << time_sec << ", extra: " << transform::DebugQuaternion(tmp) << std::endl;
  }
